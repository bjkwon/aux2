...
void put_audio_to_buf(short Buffer[2][1152], int iread, double* buf_l, double* buf_r, int nch)
{
    ...
    {
        buf_l[k] = (double)Buffer[0][k] / 0xffff;
        if (buf_r)
            buf_r[k] = (double)Buffer[1][k] / 0xffff;
    }
}

void put_audio_to_fbuf(short Buffer[2][1152], int iread, float* fbuf_l, float* fbuf_r, int nch)
{
    for (int k = 0; k < iread; k++)
    {
        fbuf_l[k] = (float)Buffer[0][k] / 0xffff; // TODO
        if (fbuf_r)
            fbuf_r[k] = (float)Buffer[1][k] / 0xffff; // TODO
    }
}
/* the simple lame decoder */
static int
lame_decoder_loop(lame_t gfp, FILE * outf, csignals_mp3_aiff* px)
{
    ...
    do {
        ...
        if (iread >= 0) {
            ...
            if (wavsize + iread > px->length)
            {
                px->length += wavsize + iread * 16;
                px->buf_l = reallocbuf(px->buf_l, px->floatbuf, wavsize, px->length);
                if (px->buf_r)
                    px->buf_r = reallocbuf(px->buf_r, px->floatbuf, wavsize, px->length);
            }
            if (px->floatbuf)
                put_audio_to_fbuf(Buffer, iread, px->fbuf_l + wavsize, px->fbuf_r ? px->fbuf_r + wavsize : NULL, tmp_num_channels);
            else
                put_audio_to_buf(Buffer, iread, px->buf_l + wavsize, px->buf_r ? px->buf_r + wavsize : NULL, tmp_num_channels);
            wavsize += iread;
        }
    } while (iread > 0);
    if (dp != 0)
...
static void
f2hei_array(const float* src, void* dest, int count, int normalize)
{
    unsigned char* ucptr;
    float			normfact;
    short				value;

    normfact = normalize ? (1.0f * 0x7FFF) : 1.0f;
    ucptr = ((unsigned char*)dest) + 4 * count;
    while (--count >= 0)
    {
        ucptr -= 4;
...
f2bei_array(const float* src, int* dest, int count, int normalize)
{
    ...
    normfact = normalize ? (1.0f * 0x7FFFFFFF) : 1.0f;
    ucptr = ((unsigned char*)dest) + 4 * count;
    while (--count >= 0)
    {
        ucptr -= 4;
...
lame_encoder_loop(lame_global_flags * gf, FILE * outf, csignals_mp3_aiff *px, char *errstr)
{
	...
	do {
        ...
        if (px->floatbuf)
        {
            f2hei_array(px->fbuf_l + cum, (void*)Buffer, iread, 1);
            if (px->buf_r)
                f2hei_array(px->fbuf_r + cum, (void*)&Buffer[1], iread, 1);
        }
        else
        {
...
int lame_aux_encode(lame_t gf, const char *filename, csignals_mp3_aiff *px, char *errstr)
{
	...
	if (lame_set_in_samplerate(gf, px->fs))
		return 0;
	if (lame_set_num_samples(gf,(unsigned long)px->length))
		return 0;
...
int lame_aux_decode(lame_t gf, const char *filename, csignals_mp3_aiff *px, char *errstr)
{
    ...
    ret = lame_decoder_loop(gf, outf, px);
    close_infile();     /* close the input file */
