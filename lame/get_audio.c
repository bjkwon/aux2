...
init_infile(lame_t gfp, char const *inPath, csignals_mp3_aiff *px)
{
    ...
    if (is_mpeg_file_format(global_reader.input_format)) {
        ...
        if (global.snd_file == 0) {
			if (global_reader.input_format == sf_aiff)
				global.music_in = open_wave_file(gfp, inPath, &enc_delay, &enc_padding);
			else
			{
				lame_set_num_samples(gfp, MAX_U_32_NUM);
				global_reader.input_format = set_basic_info(gfp, px); //returns 0 (sf_unknown for success--that's fine becuse this is used to write into mp3
...
set_basic_info(lame_global_flags * gfp, const csignals_mp3_aiff * px)
{
    global. count_samples_carefully = 0;
    global. pcm_is_unsigned_8bit = global_raw_pcm.in_signed == 1 ? 0 : 1;
	int     channels = 1 + px->buf_r != NULL;
	int     bits_per_sample = 16; // check
	if (!set_input_num_channels(gfp, channels))
		return -1;
	if (!set_input_samplerate(gfp, px->fs))
		return -1;
	global.pcmbitwidth = bits_per_sample;
	global.pcm_is_unsigned_8bit = 1;
	global.pcm_is_ieee_float = 0;
	if (px->length >= MAX_U_32_NUM)
		(void)lame_set_num_samples(gfp, MAX_U_32_NUM);
	else
		(void)lame_set_num_samples(gfp, (unsigned long)px->length); // unsigned long necessary for px->length?
...
lame_decode_initfile(FILE * fd, mp3data_struct * mp3data, int *enc_delay, int *enc_padding)
{
    ...
    hip_set_msgf(global.hip, global_ui_config.silent < 10 ? NULL : 0);
    hip_set_errorf(global.hip, global_ui_config.silent < 10 ? NULL : 0);
    hip_set_debugf(global.hip, NULL);
