...
CSignal __envelope(float* buf, unsigned int len, void* pargin, void* pargout)
{
	int fs = *(int*)pargin;
	CSignal copy(fs), copy2(fs), out(fs);
	copy.UpdateBuffer(len);
	copy2.UpdateBuffer(len);
	memcpy(copy.buf, buf, len * sizeof(float));
	memcpy(copy2.buf, buf, len * sizeof(float));
...
void _hilbenvlope(skope* past, const AstNode* pnode, const vector<CVar>& args)
{
